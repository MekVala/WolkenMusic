...
class MyUserAdmin(UserAdmin):
    ...
    def change_view(self, request, object_id, form_url='', extra_context=None):
        if extra_context is None:
            ...
            etc="select id,song_title,SUM(played_counter) AS sum from music_song where album_id in (select id from music_album where user_id = {0}) group by song_title  having sum >4 LIMIT 10".format(object_id)
            ...
            extra_context = {"try": etc,"songname": songname,"songcount": songcount}
